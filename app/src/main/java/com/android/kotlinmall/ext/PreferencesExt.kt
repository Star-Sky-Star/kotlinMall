...
class Preference<T>(val context: Context, val name: String, val default: T, val prefName: String = "default")
    ...
    private fun findPreference(key: String): T{
        return when(default){
            is Long -> with(prefs) { getLong(key, default) }
            is Int -> with(prefs) { getInt(key, default) }
            is Boolean -> with(prefs) { getBoolean(key, default) }
            is String -> with(prefs) { getString(key, default) }
